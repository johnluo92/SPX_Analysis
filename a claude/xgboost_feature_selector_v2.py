"""
Feature Selection for Simplified VIX Forecasting
Selects top N features based on importance for predicting log VIX change
Always includes cohort flags even if SHAP ranks them low
"""
import json
import logging
...
class SimplifiedFeatureSelector:
    """
    Feature selector for log VIX change prediction.
    Selects top N features and always includes cohort flags.
    """
    ...
    def __init__(self, horizon: int = 5, top_n: int = 40, cv_folds: int = 3):
        ...
        logger.info(f"Initialized Feature Selector:")
        logger.info(f"  Horizon: {horizon} days")
        logger.info(f"  Target: Log VIX change")
        logger.info(f"  Top N: {top_n}")
        logger.info(f"  CV Folds: {cv_folds}")
    
    def select_features(
        self, features_df: pd.DataFrame, vix_series: pd.Series
    ) -> Tuple[List[str], Dict]:
        """
        Select top features for predicting log VIX change.
        ...
        """
        logger.info("\n" + "=" * 80)
        logger.info("FEATURE SELECTION - LOG VIX CHANGE")
        logger.info("=" * 80)
        ...
        if len(common_dates) < 100:
            logger.error(f"❌ Insufficient aligned data: {len(common_dates)} samples")
            return [], {}
        ...
        logger.info(f"    Date range: {X.index[0].date()} to {X.index[-1].date()}")
        ...
        selected = self._select_top_features(importance_scores, X.columns)
        ...
        self.metadata = {
            ...
            "top_20_features": [
                {"feature": f, "importance": float(importance_scores[f])}
    ...
    def _calculate_target(self, vix_series: pd.Series, dates: pd.DatetimeIndex) -> pd.Series:
        """
        Calculate forward log VIX change.
        """
        target = pd.Series(index=dates, dtype=float)
        ...
        for date in dates:
            ...
            if end_pos >= len(vix_series):
                insufficient_data += 1
                continue
            
            current_vix = vix_series.iloc[date_pos]
            future_vix = vix_series.iloc[end_pos]
            
            # Skip if either is NA or invalid
            if pd.isna(current_vix) or pd.isna(future_vix):
                insufficient_data += 1
                continue
            
            if current_vix <= 0 or future_vix <= 0:
                insufficient_data += 1
                continue
            
            # Log change
            log_change = np.log(future_vix / current_vix)
            target[date] = log_change
            valid_count += 1
        ...
        logger.info(f"    Target mean: {target.mean():.4f}")
        logger.info(f"    Target std: {target.std():.4f}")
    ...
    def _select_top_features(self, importance_scores: Dict, all_features: pd.Index) -> List[str]:
        """
        Select top N features, always including cohort flags.
        """
        ...
        # Cohort flags to always include
        ...
        for cf in cohort_features:
            if cf in all_features and cf not in selected:
                selected.append(cf)
                logger.info(f"  Added cohort feature: {cf}")
        
        logger.info(f"\n  Selected {len(selected)} features:")
        logger.info(f"    Top importance: {importance_scores[selected[0]]:.6f}")
        logger.info(f"    Min importance: {importance_scores[selected[self.top_n - 1] if self.top_n <= len(selected) else selected[-1]]:.6f}")
        logger.info(f"    Cohort features included: {sum(1 for cf in cohort_features if cf in selected)}")
    ...
    def _print_summary(self, selected_features: List[str], importance_scores: Dict):
        ...
        for rank, feature in enumerate(selected_features[:20], 1):
            ...
            cohort_flag = " [COHORT]" if feature.startswith("is_") else ""
