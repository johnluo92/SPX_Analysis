"""
Sector Rotation Model v3.3 - Real Ensemble Diversity Boost
IMPROVEMENTS:
- ✅ Added real tradable securities (TLT, XME, XRT) - NO derivative ETFs
- ✅ Fixed yfinance download (proper column handling)
- ✅ Credit spreads via direct bond ETFs (not derivatives)
- ✅ Sector breadth signals (XME=metals, XRT=retail)
- ✅ Conservative config validated (leaf=40, depth=6)
- ✅ Target: Gap <0.25 for 6+ sectors

Focus: Real tradable instruments that teach the model what drives sector rotation
NO CONTANGO: No UUP, no futures-based ETFs
"""
...
class SectorRotationFeatures:
    """Feature engineering - proven v3.2 + new diversity."""
    ...
    def calculate_macro_changes(self,
        ...
        """
        Calculate macro factor changes with NEW diversity signals.
        """
        ...
        # Yield curve slope (existing, proven)
        ...
        # Gold-Dollar ratio (existing, proven)
        ...
        # NEW: Credit risk appetite (HYG/LQD) - REAL TRADABLE
        if 'HYG' in macro.columns and 'LQD' in macro.columns:
            ...
            print("   ✅ Added Credit Risk Appetite (HYG/LQD)")
        
        # NEW: Rate expectations (TLT) - REAL TRADABLE
        if 'TLT' in macro.columns:
            features['Long_Rate_Expectations'] = macro['TLT'].pct_change(63) * 100
            print("   ✅ Added Long Rate Expectations (TLT)")
        
        # NEW: Industrial metals strength (XME) - REAL TRADABLE
        if 'XME' in macro.columns:
            features['Industrial_Metals'] = macro['XME'].pct_change(63) * 100
            print("   ✅ Added Industrial Metals (XME)")
        
        # NEW: Retail health (XRT) - REAL TRADABLE
        if 'XRT' in macro.columns:
            features['Retail_Health'] = macro['XRT'].pct_change(63) * 100
            print("   ✅ Added Retail Health (XRT)")
    ...
    def combine_features(self,
        ...
        """Combine all features - clean & diverse."""
        print("\n" + "="*70)
        print("FEATURE ENGINEERING v3.3 - DIVERSITY BOOST")
        print("="*70)
...
class SectorRotationModel:
    """Model with v3.2 conservative config + v3.3 diversity."""
    
    def __init__(self, random_state: int = 42):
        """Initialize with CONSERVATIVE config (leaf=40 validated)."""
        self.config = {
            'n_estimators': 200,
            'max_depth': 6,
            'min_samples_split': 25,
            'min_samples_leaf': 40,  # Validated: reduces overfitting
            'max_features': 'sqrt',
        }
...
def fetch_tier1_data(start_str: str, end_str: str) -> pd.DataFrame:
    """
    Fetch Tier 1 independent variables (REAL TRADABLE).
    NO DERIVATIVES: Only direct equity/bond ETFs.
    """
    print("\n🔧 Fetching Tier 1 tradable instruments...")
    
    tier1_tickers = {
        'HYG': 'HYG',     # High Yield Corp Bonds (tradable)
        'LQD': 'LQD',     # Investment Grade Bonds (tradable)
        'TLT': 'TLT',     # 20Y Treasury ETF (tradable, rate expectations)
        'XME': 'XME',     # Metals & Mining (industrial metals signal)
        'XRT': 'XRT',     # Retail ETF (consumer health signal)
    }
...
def test_v33_model():
    """Test v3.3 - real ensemble diversity boost."""
    print("\n" + "="*70)
    print("SECTOR ROTATION MODEL v3.3 - REAL DIVERSITY BOOST")
    print("="*70)
    ...
    if not tier1_df.empty:
        ...
        print(f"\n✅ Added {len(tier1_df.columns)} Tier 1 tradable instruments")
    else:
        print("\n⚠️  No Tier 1 data fetched - continuing with base macro only")
    ...
    # Feature engineering
    print("\n📊 Step 2: Feature Engineering (v3.3 - Diversity)")
    print("-"*70)
    ...
    print("\n" + "="*70)
    print("✅ v3.3 TEST COMPLETE - REAL DIVERSITY ADDED")
    print("="*70)
    print("\n💡 New Features (ALL TRADABLE):")
    print("   ✅ HYG/LQD ratio (credit risk appetite)")
    print("   ✅ TLT (20Y Treasury, rate expectations)")
    print("   ✅ XME (metals/mining, industrial signal)")
    print("   ✅ XRT (retail, consumer health)")
    print("   🎯 Goal: 6+ sectors with gap <0.25")
    
    return model, features, results, validation, confidence


if __name__ == "__main__":
    model, features, results, validation, confidence = test_v33_model()