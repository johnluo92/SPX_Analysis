"""
Sector Rotation Model v3.6 - Back to Basics (The XLF Fix)
ROLLBACKS from v3.3/v3.4/v3.5:
- ❌ Remove HYG, LQD, TLT, XME, XRT (multicollinearity with sectors)
- ❌ Back to 7 years of data (2018-2025, stable regime)
- ❌ Simpler regularization (leaf=30, depth=6)
- ❌ Remove interaction terms (Slope×Credit created confusion)
- ✅ Keep only CORE 5 factors (Gold, Oil, Dollar, 10Y, 2Y)
- ✅ Keep proven windows (63d, 126d for RS; 63d for macro)
- ✅ Keep VIX features (volatility proven important)
- ✅ Simple feature selection (1.5% threshold)

Philosophy: "Simplicity is the ultimate sophistication" - da Vinci
Goal: Get XLF back to 60%+ test accuracy, <0.30 gap
"""
...
class SectorRotationFeaturesV6:
    """BACK TO BASICS: Core 5 factors only."""
    ...
    def calculate_macro_changes(self,
        ...
        """Calculate macro changes - CORE 5 ONLY."""
        ...
        # Gold-Dollar ratio (proven in v2.0)
    ...
    def add_vix_features(self, vix: pd.Series) -> pd.DataFrame:
        """Add VIX features - proven in v3.x."""
    ...
    def combine_features(self,
        ...
        """Combine all features - SIMPLE."""
        print("\n" + "="*70)
        print("FEATURE ENGINEERING v3.6 - BACK TO BASICS")
        print("="*70)
...
class SectorRotationModelV6:
    """Model v3.6 - Simple & effective."""
    
    def __init__(self, random_state: int = 42):
        """Initialize with BALANCED config."""
        
        # BALANCED regularization (not too tight, not too loose)
        self.base_config = {
            'n_estimators': 200,
            'max_depth': 6,
            'min_samples_split': 20,
            'min_samples_leaf': 30,  # Sweet spot
            'max_features': 'sqrt',
        }
    ...
    def train_models(self,
        ...
        """Train models with balanced config."""
        print("\n" + "="*70)
        print("TRAINING MODELS v3.6 - SIMPLE & CALIBRATED")
        print("="*70)
...
def test_v36_rollback():
    """Test v3.6 - Back to basics (The XLF Fix)."""
    print("\n" + "="*70)
    print("SECTOR ROTATION MODEL v3.6 - BACK TO BASICS")
    print("="*70)
    
    from sector_data_fetcher import SectorDataFetcher
    from UnifiedDataFetcher import UnifiedDataFetcher
    
    # Fetch data (7 years - stable regime)
    ...
    print(f"   Rationale: 2018-2025 = stable post-crisis regime")
    print(f"   NO pre-2018 ZIRP data (hurts XLF)")
    ...
    # NO TIER 1 INSTRUMENTS (removed HYG, LQD, TLT, XME, XRT)
    print("\n⚠️  REMOVED Tier 1 instruments (HYG, LQD, TLT, XME, XRT)")
    print("   Reason: Created multicollinearity with sectors")
    ...
    # Feature engineering
    print("\n📊 Step 2: Feature Engineering (v3.6 - Simple)")
    print("-"*70)
    
    feat_eng = SectorRotationFeaturesV6()
    ...
    # XLF focus
    print("\n" + "="*70)
    print("🎯 XLF (FINANCIALS) PERFORMANCE CHECK")
    print("="*70)
    
    xlf_acc = results_df.loc['XLF', 'test_accuracy']
    xlf_gap = results_df.loc['XLF', 'overfitting_gap']
    xlf_wf_std = validation['XLF']['std_accuracy']
    
    print(f"\nXLF Results:")
    print(f"   Test Accuracy: {xlf_acc:.3f}")
    print(f"   Overfitting Gap: {xlf_gap:.3f}")
    print(f"   Walk-Forward Std: {xlf_wf_std:.3f}")
    
    if xlf_acc > 0.55 and xlf_gap < 0.30:
        print(f"\n✅ XLF FIXED! Back to usable performance")
    else:
        print(f"\n🟡 XLF still needs work")
    ...
    print("\n" + "="*70)
    print("✅ v3.6 COMPLETE - BACK TO BASICS")
    print("="*70)
    print("\n🎯 v3.6 Rollbacks:")
    print("   ❌ Removed HYG, LQD, TLT, XME, XRT (multicollinearity)")
    print("   ❌ Removed interaction terms (Slope×Credit, Energy×Metals)")
    print("   ❌ Back to 7 years (2018-2025 only)")
    print("   ❌ Simpler regularization (leaf=30, not 40-60)")
    print("   ✅ Kept CORE 5 factors (Gold, Oil, Dollar, 10Y, 2Y)")
    print("   ✅ Kept proven windows (63d, 126d)")
    print("   ✅ Kept VIX features")
    print("   🎯 Goal: Restore XLF performance\n")
    
    return model, features, results, validation, confidence


if __name__ == "__main__":
    model, features, results, validation, confidence = test_v36_rollback()