...
# ============================================================================
# TEMPORAL VALIDATION DATES
# ============================================================================
TRAINING_END_DATE = "2022-12-31"
CALIBRATION_PERIOD = ("2023-01-01", "2024-06-30")
...
TARGET_CONFIG = {
    "horizon_days": 5,
    "horizon_label": "5d",
    "target_type": "log_vix_change",
    "output_type": "vix_pct_change",
    "log_space": {
        "enabled": True,
        "description": "Train on log(future_vix/current_vix), convert to % for output"
    },
    "movement_bounds": {
        "floor": -50.0,
        "ceiling": 100.0,
        "description": "Percentage change bounds (converted from log-space)"
    }
}

# ============================================================================
# CALENDAR COHORTS - USED AS FEATURES
# ============================================================================
CALENDAR_COHORTS = {
    "fomc_period": {
...
# ============================================================================
# XGBOOST CONFIGURATION - SIMPLIFIED TO 2 MODELS
# ============================================================================
XGBOOST_CONFIG = {
    ...
    "objectives": {
        "direction_5d": {
            ...
            "num_classes": 2,
            "description": "Binary: VIX up (1) vs down (0) based on log-space target"
        },
        "magnitude_5d": {
            "objective": "reg:squarederror",
            "eval_metric": "rmse",
            "early_stopping_rounds": 50,
            "description": "Continuous: log-space VIX change (converted to % for output)"
        }
    },
    "cv_config": {
...
# Model objective names that other files reference
MODEL_OBJECTIVES = list(XGBOOST_CONFIG["objectives"].keys())

# ============================================================================
# PREDICTION DATABASE CONFIGURATION - SIMPLIFIED SCHEMA
# ============================================================================
PREDICTION_DB_CONFIG = {
    ...
    "schema": {
        ...
        "cohort_weight": "REAL",
        # Simplified: only direction and magnitude
        "prob_up": "REAL",
        "prob_down": "REAL",
...
PUBLICATION_LAGS = {
    "^GSPC": 0, "^VIX": 0, "CL=F": 0, "GC=F": 0, "DX-Y.NYB": 0,
    "SKEW": 0, "VIX3M": 0, "VX1-VX2": 0, "VX2-VX1_RATIO": 0,
    "CL1-CL2": 0, "DX1-DX2": 0, "COR1M": 0, "COR3M": 0,
    "VXTH": 0, "VXTLT": 0, "PCCE": 0, "PCCI": 0, "PCC": 0,
    "DGS1MO": 1, "DGS3MO": 1, "DGS6MO": 1, "DGS1": 1, "DGS2": 1,
    "DGS5": 1, "DGS10": 1, "DGS30": 1, "DTWEXBGS": 1, "CPIAUCSL": 14
}
...
# ============================================================================
# REGIME BOUNDARIES (for reference/features only)
# ============================================================================
...
# ============================================================================
# FORECASTING CONFIG (for backward compatibility)
# ============================================================================
FORECASTING_CONFIG = {
    "horizon_days": TARGET_CONFIG["horizon_days"],
    "target_type": TARGET_CONFIG["target_type"],
    "movement_bounds": TARGET_CONFIG["movement_bounds"],
...
# ============================================================================
# FEATURE GROUPS (from original config - kept for compatibility)
# ============================================================================
VIX_BASE_FEATURES = {
    "mean_reversion": [
    ...
    "regimes": [
        "vix_regime", "vix_regime_duration", "vix_displacement", "vix_term_structure"
    ]
}
...
SPX_BASE_FEATURES = {
    ...
    "ohlc_microstructure": [
        ...
        "spx_body_to_range", "spx_gap", "spx_gap_magnitude",
...
CBOE_BASE_FEATURES = {
    "skew_indicators": [
        "SKEW", "skew_regime", "skew_vs_vix", "skew_vix_ratio", "skew_displacement"
    ],
    "put_call_ratios": [
        "pc_equity_inst_divergence", "pcc_accel_10d"
    ],
    "correlation_indices": [
    ...
    "other_cboe": [
        "VXTH", "VXTH_change_21d", "VXTH_zscore_63d", "vxth_vix_ratio",
        "cboe_stress_composite", "cboe_stress_regime"
    ],
    "bond_volatility": [
        "VXTLT", "VXTLT_change_21d", "VXTLT_zscore_63d",
        "VXTLT_velocity_10d", "VXTLT_acceleration_5d", "bond_vol_regime",
        "vxtlt_vix_ratio", "vxtlt_vix_spread",
...
META_FEATURES = {
    "regime_indicators": [
        "vix_regime_micro", "regime_transition_risk",
        "vol_regime", "risk_premium_regime", "vol_term_regime",
        "trend_regime", "trend_strength",
        "liquidity_stress_composite", "liquidity_regime", "correlation_regime"
    ],
    "cross_asset_relationships": [
        "equity_vol_divergence", "equity_vol_corr_breakdown",
        "risk_premium_ma21", "risk_premium_velocity", "risk_premium_zscore",
        "gold_spx_divergence", "dollar_spx_correlation"
    ],
    "rate_of_change": [
        "vix_velocity_3d_pct", "vix_jerk_5d", "vix_momentum_regime",
    ...
    "percentile_rankings": [
        ...
        "vix_extreme_low_63d", "vix_extreme_low_252d",
...
MACRO_FEATURES = {
    ...
    "volatility_indices": ["Bond_Vol_lag1", "Bond_Vol_mom_10d", "Bond_Vol_mom_21d",
...
# ============================================================================
# FEATURE CONFIG (for compatibility with feature_engineer)
# ============================================================================
FEATURE_CONFIG = {
    "base_features": {
