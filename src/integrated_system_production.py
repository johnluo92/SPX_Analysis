...
class IntegratedForecastingSystem:
    ...
    def generate_forecast(self,date=None,store_prediction=True):
        ...
        if target_date not in df.index:available_range=f"{df.index[0].date()} to {df.index[-1].date()}";raise ValueError(f"Date {target_date.date()} not in feature data. Available: {available_range}")
        ...
        if self.calibrator.fitted:
            ...
            forecast["calibration"]={"correction_type":"not_fitted","adjustment":0.0}
    ...
    def _log_forecast_summary(self,forecast):
        ...
        if "calibration"in forecast:cal=forecast["calibration"];logger.info(f"  Calibration:");logger.info(f"  Adjustment: {cal.get('adjustment',0):+.3f}%");logger.info(f"  Method: {cal.get('correction_type','none')}\n")
    ...
    def generate_forecast_batch(self,start_date,end_date,prebuilt_features=None):
        ...
        if prebuilt_features is None:logger.info("🔧 Building features...");feature_data=self.feature_engine.build_complete_features(years=TRAINING_YEARS,end_date=end_date,force_historical=True);df=feature_data["features"];metadata_cols=["calendar_cohort","cohort_weight","feature_quality"];numeric_cols=[c for c in df.columns if c not in metadata_cols];[df.__setitem__(col,pd.to_numeric(df[col],errors="coerce").fillna(0.0))if df[col].dtype==object else None for col in numeric_cols];[df.__setitem__(col,df[col].astype(np.float64))for col in numeric_cols];logger.info("✅ Features validated")
        ...
        for i,date in enumerate(date_range):
            try:
                ...
                if not usable:logger.warning(f"   Skipped {date.date()}: low quality");continue
                feature_values=observation[self.forecaster.feature_names];feature_array=pd.to_numeric(feature_values,errors="coerce").values;X_df=pd.DataFrame(feature_array.reshape(1,-1),columns=self.forecaster.feature_names,dtype=np.float64).fillna(0.0);current_vix=float(observation["vix"]);forecast=self.forecaster.predict(X_df,current_vix)
                ...
                forecast["metadata"]={"observation_date":date.strftime("%Y-%m-%d"),"forecast_date":forecast_date.strftime("%Y-%m-%d"),"horizon_days":TARGET_CONFIG["horizon_days"],"feature_quality":float(quality_score),"cohort_weight":float(observation.get("cohort_weight",1.0)),"calendar_cohort":cohort,"features_used":len(self.forecaster.feature_names)}
        ...
        if status["pending_writes"]>0:logger.error(f"🚨 WARNING: {status['pending_writes']} uncommitted!");raise RuntimeError(f"Lost {status['pending_writes']} forecasts!")
        return forecasts
    def _get_features(self,force_refresh=False):
        now=pd.Timestamp.now().normalize();today=(now.year,now.month,now.day)
        if not force_refresh and self._feature_cache is not None and self._feature_cache_date==today:logger.info("📦 Using cached features");return self._feature_cache
        ...
        return df_clean
def main():
    parser=argparse.ArgumentParser(description="Integrated VIX Forecasting System V5.1")
    parser.add_argument("--mode",choices=["forecast","complete","batch","backfill"],default="forecast")
    parser.add_argument("--forecast-date",type=str)
    parser.add_argument("--start-date",type=str)
    parser.add_argument("--end-date",type=str)
    parser.add_argument("--models-dir",type=str,default="models")
    parser.add_argument("--db-path",type=str,default="data_cache/predictions.db")
    args=parser.parse_args()
    Path("logs").mkdir(exist_ok=True);system=IntegratedForecastingSystem(models_dir=args.models_dir,db_path=args.db_path)
    if args.mode=="forecast":forecast_date=pd.Timestamp(args.forecast_date)if args.forecast_date else None;result=system.generate_forecast(date=forecast_date);sys.exit(0 if result else 1)
        ...
        try:
            ...
            logger.info(f"[2/8] Calibration forecasts ({cal_start} to {cal_end})...");system.generate_forecast_batch(cal_start,cal_end,prebuilt_features=prebuilt_df)
            logger.info(f"[3/8] Backfilling actuals (calibration)...");system.prediction_db.backfill_actuals()
            logger.info(f"[4/8] Validation forecasts ({val_start} to {val_end})...");system.generate_forecast_batch(val_start,val_end,prebuilt_features=prebuilt_df)
            logger.info(f"[5/8] Backfilling actuals (validation)...");system.prediction_db.backfill_actuals()
            logger.info(f"[6/8] Production forecasts ({PRODUCTION_START_DATE} to {today})...");system.generate_forecast_batch(PRODUCTION_START_DATE,today,prebuilt_features=prebuilt_df)
            logger.info(f"[7/8] Backfilling actuals (production)...");system.prediction_db.backfill_actuals()
            logger.info(f"[8/8] Running diagnostics...")
            ...
            except ImportError:logger.warning("Walk-forward validation unavailable")
            logger.info("  • diagnostics/walk_forward_metrics.json")
            logger.info(f"📈 Calibration: {cal_start} to {cal_end}")
            logger.info(f"📈 Validation: {val_start} to {val_end}")
            logger.info(f"📈 Production: {PRODUCTION_START_DATE} to {today}")
