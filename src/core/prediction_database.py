...
class PredictionDatabase:
    def __init__(self,db_path=None):
        ...
        self.db_path=Path(db_path);self.db_path.parent.mkdir(parents=True,exist_ok=True);self.conn=sqlite3.connect(self.db_path,check_same_thread=False);self.conn.row_factory=sqlite3.Row;self._create_schema();self._pending_keys=set();self._commit_tracker=CommitTracker();atexit.register(self._commit_tracker.verify_clean_exit)
    ...
    def store_prediction(self,record):
        ...
        if key in self._pending_keys:
            logger.debug(f"Prediction already pending for {record['forecast_date']} (horizon={record['horizon']}). Skipping.");return None
        ...
        try:
            ...
            if existing:
                logger.debug(f"Prediction already exists for {record['forecast_date']} (horizon={record['horizon']}). Skipping.");self._pending_keys.discard(key);return None
            columns=list(record.keys());placeholders=", ".join(["?"for _ in columns]);insert_sql=f"INSERT INTO forecasts ({', '.join(columns)}) VALUES ({placeholders})";values=[record[col]for col in columns];self.conn.execute(insert_sql,values);self._commit_tracker.track_write(f"INSERT forecast_date={record['forecast_date']}");return record["prediction_id"]
        except sqlite3.IntegrityError as e:
            logger.error(f"❌ Duplicate or constraint violation: {e}");self.conn.rollback();self._pending_keys.discard(key);return None
        except Exception as e:
            logger.error(f"❌ Failed to store prediction: {e}");logger.error(f"   Record: {record.get('forecast_date')}, horizon={record.get('horizon')}");logger.error(f"   Available keys: {list(record.keys())[:10]}...");self.conn.rollback();self._pending_keys.discard(key);raise
    def commit(self):
        if self._commit_tracker.pending_writes==0:return
        writes_to_commit=self._commit_tracker.pending_writes
        try:
            self.conn.commit();self._pending_keys.clear();cursor=self.conn.execute("SELECT COUNT(*) FROM forecasts");total=cursor.fetchone()[0];logger.info(f"✅ Committed {writes_to_commit} predictions (total: {total})");self._commit_tracker.pending_writes=0;self._commit_tracker.writes_log=[];self._commit_tracker.last_commit_time=datetime.now()
        except Exception as e:
            logger.error("="*80);logger.error("🚨 COMMIT FAILED!");logger.error("="*80);logger.error(f"   Error: {e}");logger.error(f"   Attempted writes: {writes_to_commit}");logger.error("   Rolling back...");self.conn.rollback();self._commit_tracker.pending_writes=0;self._commit_tracker.writes_log=[];logger.error("="*80);raise RuntimeError(f"Database commit failed: {e}")
    ...
    def get_predictions(self,start_date=None,end_date=None,cohort=None,with_actuals=False):
        ...
        try:
            ...
            logger.error(f"❌ Failed to query predictions: {e}");raise
    def backfill_actuals(self,fetcher=None):
        ...
        for row in rows:
            ...
            except Exception as e:logger.warning(f"   Failed to calculate business days for {forecast_date}: {e}");skipped+=1;continue
            found_date=None
