...
class PredictionDatabase:
    ...
    def store_prediction(self,record:Dict)->Optional[str]:
        ...
        try:
            cursor=self.conn.execute("SELECT prediction_id FROM forecasts WHERE forecast_date = ? AND horizon = ?",(record["forecast_date"],record["horizon"]))
            existing=cursor.fetchone()
            if existing:
                logger.warning(f"⚠️  Prediction already exists for {record['forecast_date']} (horizon={record['horizon']}). Skipping.")
    ...
    def get_predictions(self,start_date:str=None,end_date:str=None,cohort:str=None,with_actuals:bool=False)->pd.DataFrame:
