...
class CommitTracker:
    ...
    def track_write(self, operation):
        ...
        if self.pending_writes % 10 == 0:
            logger.debug(f"⚠️  {self.pending_writes} uncommitted writes! Call commit() soon!")
...
class PredictionDatabase:
    ...
    def commit(self):
        if self._commit_tracker.pending_writes == 0:
            return  # Silent if nothing to commit
        ...
        try:
            ...
            logger.info(f"✅ Committed {writes_to_commit} predictions (total: {total})")
