...
class AnomalyDatabaseExtension:
    ...
    def backfill_anomaly_scores(self,anomaly_scorer,feature_data):
        ...
        for date in anomaly_results.index:
            ...
            vix_value=feature_data.loc[date,'vix']if'vix'in feature_data.columns else None
            vix_vel=feature_data.loc[date,'vix_velocity_5d']if'vix_velocity_5d'in feature_data.columns else None
            vix_accel=feature_data.loc[date,'vix_accel_5d']if'vix_accel_5d'in feature_data.columns else None
            scorer_result=anomaly_scorer.calculate_score(feature_data.loc[[date]])
            record={'observation_date':obs_date,'anomaly_score':float(anomaly_results.loc[date,'anomaly_score']),'score_level':anomaly_results.loc[date,'anomaly_level'],'vix_value':float(vix_value)if vix_value is not None else None,'vix_velocity_5d':float(vix_vel)if vix_vel is not None else None,'vix_accel_5d':float(vix_accel)if vix_accel is not None else None,'top_contributing_features':scorer_result.get('contributing_features',[]),'detector_scores':{}}
