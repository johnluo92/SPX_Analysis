...
class ProbabilisticBacktester:
    ...
    def evaluate_quantile_coverage(self, df):
        ...
        for q in [10, 25, 50, 75, 90]:
            ...
            logger.info(
                f"   {col}: {empirical:.3f} (nominal: {nominal:.2f}, error: {error:+.3f}) {status}"
            )
    ...
    def evaluate_brier_score(self, df):
        ...
        logger.info(
            f"   Brier Score: {mean_brier:.3f} ({'✅ Good' if mean_brier < 0.2 else '⚠️ Poor'})"
        )
    ...
    def evaluate_by_cohort(self, df):
            ...
            cohort_metrics[cohort] = {
                ...
                "rmse": float(
                    np.sqrt(
                        (
                            (
                                cohort_df["actual_vix_change"]
                                - cohort_df["point_estimate"]
                            )
                            ** 2
                        ).mean()
                    )
        ...
        for cohort, metrics in sorted_cohorts:
            logger.info(
                f"   {cohort:<30} | {metrics['n']:>5} | {metrics['mae']:>7.2f}% | {metrics['rmse']:>7.2f}% | {metrics['brier']:>8.3f}"
            )
    ...
    def plot_diagnostics(self, df, save_dir):
        ...
        empirical = [
            self.results["quantile_coverage"][f"q{q}"]["empirical"] for q in quantiles
        ]
        ...
        axes[0, 1].set_title(
            f"Confidence vs Error (ρ={self.results['confidence']['correlation']:.2f})"
        )
        ...
        errors_by_cohort = [
            df[df["calendar_cohort"] == c]["point_error"].values for c in cohorts
        ]
        ...
        df_sorted["brier"] = df_sorted.apply(
            lambda row: (
    ...
    def print_summary(self):
        ...
        for q in [10, 25, 50, 75, 90]:
            ...
            logger.info(
                f"   {q}th: {metrics['empirical']:.1%} (nominal: {metrics['nominal']:.0%}) {status}"
            )
        ...
        logger.info(
            f"⚠️ Worst Cohort: {sorted_cohorts[-1][0]} (MAE: {sorted_cohorts[-1][1]['mae']:.2f}%)"
        )
