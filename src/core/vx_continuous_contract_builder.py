...
class VXContinuousContractBuilder:
    ...
    def _compute_vx_expiry(cls,year,month):
        ...
        third_fri=date(next_year,next_month,fridays[2])
        return third_fri-timedelta(days=30)

    @classmethod
    def _build_expiration_dates(cls,through_year=2028,through_month=12):
        # Start from the anchor table, then append dynamically computed entries
        # for any (year, month) not already covered.
        anchor_set={(y,m)for y,m,_ in cls._EXPIRATION_DATES_ANCHOR}
        result=list(cls._EXPIRATION_DATES_ANCHOR)
        last_anchor_year,last_anchor_month,_=cls._EXPIRATION_DATES_ANCHOR[-1]
        y,m=last_anchor_year,last_anchor_month
        while True:
            m+=1
            if m>12:m=1;y+=1
            if y>through_year or(y==through_year and m>through_month):break
            if(y,m)in anchor_set:continue
            d=cls._compute_vx_expiry(y,m)
            result.append((y,m,d.strftime('%Y-%m-%d')))
        return result

    # EXPIRATION_DATES is the full table: anchor + dynamic extension through 2028-12
    EXPIRATION_DATES=None  # populated after class body via _build_expiration_dates()
    ...
    def _fetch_missing_cboe_data(self,date):
        ...
        for fn in missing:
            ...
            try:
                ...
                print(f"   ⚠ CBOE fetch failed for {fn}: {str(e)[:60]}")
            except Exception as e:
                print(f"   ⚠ Unexpected error fetching {fn}: {type(e).__name__}")
        return len(missing)
    ...
    def _load_raw_contract(self,fn):
        ...
        try:
            ...
            if 'close'in df.columns:df.loc[zero_settle,'settle']=df.loc[zero_settle,'close']
            valid_settle=(df['settle']>0)&df['settle'].notna()
            if valid_settle.sum()==0:return pd.DataFrame()
            df=df[valid_settle].copy()
            if 'close'in df.columns:
                zero_close=(df['close']<=0)|df['close'].isna();df.loc[zero_close,'close']=df.loc[zero_close,'settle']
            if 'open'in df.columns:
                zero_open=(df['open']<=0)|df['open'].isna();df.loc[zero_open,'open']=df.loc[zero_open,'settle']
            if 'high'in df.columns and 'low'in df.columns:
                invalid_hl=(df['high']<=0)|(df['low']<=0)|df['high'].isna()|df['low'].isna()
                df.loc[invalid_hl,'high']=df.loc[invalid_hl,'settle'];df.loc[invalid_hl,'low']=df.loc[invalid_hl,'settle']
            ...
            for c in['volume','open_interest']:
                if c in df.columns:
                    neg_or_nan=(df[c]<0)|df[c].isna();df.loc[neg_or_nan,c]=0
            corrupted=(df['settle']<3)|(df['settle']>300)
            if corrupted.any():
                print(f"   ⚠ Corrupted data in {fn}: {df[corrupted]['settle'].values}")
                df=df[~corrupted]
            return df
        except Exception as e:
            print(f"   ✗ Failed to load {fn}: {e}");return pd.DataFrame()
    ...
    def _build_continuous_incremental(self,pos,start_date=None):
        ...
        return self._tag_roll_events(combined)

    def _tag_roll_events(self,df):
        # Drops raw roll tracking columns and adds roll_indicator / contract_id labels.
        # Does NOT back-adjust prices (no Panama or ratio adjustment).
        if 'roll_event'not in df.columns:return df
        work=df.copy();work['roll_indicator']=work['roll_event'];work['contract_id']=work['roll_event'].cumsum()
...
# Populate the combined expiry table (anchor 2013-2026-08 + computed through 2028-12)
VXContinuousContractBuilder.EXPIRATION_DATES=VXContinuousContractBuilder._build_expiration_dates(through_year=2028,through_month=12)
