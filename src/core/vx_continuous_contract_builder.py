...
class VXContinuousContractBuilder:
    ...
    def _build_expiry_lookup(self):
        ...
        df=pd.DataFrame(records);return df.sort_values('expiry_date').reset_index(drop=True)

    def _get_cache_metadata(self,pos):
        cache_file=self.cache_dir/f"VX{pos}.parquet"
        if not cache_file.exists():return None,None
        try:df=pd.read_parquet(cache_file);return(df.index[-1],cache_file.stat().st_mtime)if not df.empty else(None,None)
    ...
    def _build_continuous_incremental(self,pos,start_date=None):
        ...
        if cached.empty:start_date=start_date or pd.Timestamp('2013-03-25')
        ...
        for d in dates:
            ...
            row=df.loc[d];dte=(contract['expiry_date']-d).days
            new_data.append({'date':d,'open':row['open'],'high':row['high'],'low':row['low'],'close':row['close'],'settle':row['settle'],'volume':row['volume'],'open_interest':row['open_interest'],'contract_code':contract['contract_code'],'expiry_date':contract['expiry_date'],'days_to_expiry':dte})
        ...
        return self._apply_backwards_ratio_adjustment(combined)
    ...
    def _apply_backwards_ratio_adjustment(self,df):
        ...
        for i in range(len(rolls)-1,-1,-1):
            roll_date=rolls[i];new_settle=work.loc[roll_date,'settle'];prev_dates=work.index[work.index<roll_date]
            if len(prev_dates)==0:continue
            prev_date=prev_dates[-1];old_settle=work.loc[prev_date,'settle']
            if old_settle>0 and new_settle>0:
                ratio=new_settle/old_settle;mask=work.index<roll_date
                for c in['open','high','low','close','settle']:
                    if c in work.columns:work.loc[mask,c]*=ratio
        work['roll_indicator']=work['roll_event'];return work.drop(columns=['roll_event','contract_code','expiry_date'],errors='ignore')
    ...
    def get_all_continuous_contracts(self,start_date=None,end_date=None,force_rebuild=False):
        ...
        print(f"   ✓ Loaded {len(contracts)} contracts");return contracts
