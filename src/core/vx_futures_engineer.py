...
class VXFuturesEngineer:
    def __init__(self, cboe_vx_dir="./CBOE_VX_ALL", cache_dir="./data_cache/vx_continuous"):
        self.cboe_vx_dir = cboe_vx_dir
        self.cache_dir = cache_dir

    def build_all_vx_features(self, start_date, end_date, target_index=None, vix_series=None):
        """Build focused VX futures features for 5-day VIX prediction

        Args:
            start_date: Start date for data
            end_date: End date for data
            target_index: Optional index to align features to
            vix_series: Optional VIX spot series for basis calculations
        """
        print("   → Building VX futures features from cached contracts...")
        vx_contracts = get_vx_continuous_contracts(
            start_date=start_date,
            end_date=end_date,
            cboe_vx_dir=self.cboe_vx_dir,
            cache_dir=self.cache_dir
        )

        if not vx_contracts:
            raise ValueError("Failed to load VX continuous contracts")
        ...
        for name, df in vx_contracts.items():
            if not df.empty:
                if 'settle' in df.columns:
                    vx_prices[name] = df['settle'].reindex(common_index)
                if 'days_to_expiry' in df.columns:
                    vx_dte[name] = df['days_to_expiry'].reindex(common_index)
                if 'volume' in df.columns:
                    vx_volume[name] = df['volume'].reindex(common_index)
                if 'open_interest' in df.columns:
                    vx_oi[name] = df['open_interest'].reindex(common_index)
        ...
        if target_index is not None:
            features = features.reindex(target_index, method='ffill', limit=5)
    ...
    def _add_positioning_features(self, features, vx_oi, vx_volume):
        ...
        if 'VX1' in vx_oi:
            ...
            oi_change_5d = vx1_oi.diff(5)
            oi_mean = oi_change_5d.rolling(63).mean()
            oi_std = oi_change_5d.rolling(63).std()
    ...
    def _add_roll_features(self, features, vx_prices, vx_dte):
        """Roll yield and time decay features"""
    ...
    def _add_vix_basis_features(self, features, vx_prices, vix_series):
        """VIX spot vs VX futures basis features"""

        if 'VX1' not in vx_prices:
            return features

        vx1 = vx_prices['VX1']
        vix_aligned = vix_series.reindex(features.index)
    ...
    def _add_regime_features(self, features, vx_prices):
        """Regime classification and structural break indicators"""
        ...
        if 'vx_contango_intensity' in features:
            ...
            contango_5d = contango.diff(5)
            features['vx_regime_shift_velocity'] = contango_5d

        if 'VX1' in vx_prices:
            vx1 = vx_prices['VX1']
            vx1_ret_5d = vx1.pct_change(5)

            ret_mean = vx1_ret_5d.rolling(126).mean()
            ret_std = vx1_ret_5d.rolling(126).std()
    ...
    def _zscore(self, series, window):
        """Calculate z-score with robust handling"""
...
def build_vx_futures_features(start_date, end_date, cboe_vx_dir="./CBOE_VX_ALL", target_index=None, vix_series=None):
    """Convenience function to build VX futures features

    Args:
        start_date: Start date
        end_date: End date
        cboe_vx_dir: Directory containing CBOE VX data
        target_index: Optional index to align to
        vix_series: Optional VIX spot series for basis calculations
    """
