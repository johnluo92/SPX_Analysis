...
2. median_error COMPUTED on-the-fly (not stored in DB)
...
class EnhancedWalkForwardValidator:
    ...
    def load_predictions_with_actuals(self) -> pd.DataFrame:
        ...
        - Computes median_error on-the-fly (not stored in DB)
        ...
        # Compute median_error on-the-fly
    ...
    def _compute_overall_metrics(self, df: pd.DataFrame) -> Dict:
        ...
        V3: Uses median_error as primary metric
        """

        # Use median_error (already computed in load_predictions_with_actuals)
