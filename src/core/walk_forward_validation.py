...
class SimplifiedWalkForwardValidator:
    ...
    def compute_metrics(self,df):
        metrics={"overall":self._compute_overall_metrics(df),"direction":self._compute_direction_metrics(df),"magnitude":self._compute_magnitude_metrics(df),"by_cohort":self._compute_by_cohort(df),"time_series":self._compute_time_series_metrics(df)}
        return metrics
    ...
    def _compute_magnitude_metrics(self,df):
        ...
        return {"mae":float(mae),"rmse":float(rmse),"bias":float(bias),"median_error":float(median_error)}
    ...
    def _plot_direction_calibration(self,df,output_dir):
        ...
        ax=axes[0]
        prob_true,prob_pred=calibration_curve(df["actual_direction"],df["prob_up"],n_bins=10)
    ...
    def _plot_magnitude_accuracy(self,df,output_dir):
        ...
        lims=[min(df["magnitude_forecast"].min(),df["actual_vix_change"].min()),max(df["magnitude_forecast"].max(),df["actual_vix_change"].max())]
        ...
        ax.axvline(errors.mean(),color="blue",linestyle="--",linewidth=2,label=f"Mean: {errors.mean():.2f}%")
    ...
    def _plot_performance_by_cohort(self,df,output_dir):
        ...
        for cohort in cohorts:
            ...
            if len(cohort_df)>=5:
                ...
                maes.append(mae);labels.append(cohort)
    ...
    def _plot_time_series(self,df,output_dir):
        ...
        else:ax.plot(df["forecast_date"],df["magnitude_error"],"o-",linewidth=2)
        ...
        else:ax.plot(df["forecast_date"],df["direction_correct"],"o-",linewidth=2)
