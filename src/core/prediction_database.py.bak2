...
class PredictionDatabase:
    ...
    Schema:
        - forecasts: Main table with predictions
    ...
    def store_prediction(self, record: Dict) -> Optional[str]:
        ...
        ✅ FIX: Removed premature quantile validation - models should output valid quantiles.
        ...
        # ✅ FIX: Ensure timestamp is NOT NULL
    ...
    def get_performance_summary(self) -> Dict:
        ...
        summary = {
            ...
            "point_estimate": {
                ...
                "rmse": float(
                    np.sqrt(
                        ((df["actual_vix_change"] - df["point_estimate"]) ** 2).mean()
                    )
                ),
            },
            ...
            if "confidence_score" in df.columns and "point_error" in df.columns
            else None,
        }
