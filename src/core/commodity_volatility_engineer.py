...
class CommodityVolatilityEngineer:
    def __init__(self,data_fetcher):
        ...
        self.yahoo_symbols={'DX-Y.NYB':'Dollar_Index','GC=F':'Gold_Futures','CL=F':'Crude_Oil','NG=F':'Natural_Gas','HG=F':'Copper'}
        self.cboe_vol_indices={'^OVX':'OVX','^GVZ':'GVZ'}
    def build_all_commodity_vol_features(self,start_date:str,end_date:str,target_index:pd.DatetimeIndex,vix_series:Optional[pd.Series]=None)->pd.DataFrame:
        ...
        cross_asset_features=self._build_cross_asset_features(yahoo_data,cboe_vol_data,vix_series,target_index)
        ...
        for col in features.columns:
            if features[col].dtype=='object'or features[col].dtype.name=='category':features[col]=pd.to_numeric(features[col],errors='coerce')
        ...
        return features
    def _fetch_yahoo_data(self,start_date:str,end_date:str,target_index:pd.DatetimeIndex)->Dict[str,pd.Series]:
        data={};ff_limit=5
        for symbol,name in self.yahoo_symbols.items():
            try:
                df=self.fetcher.fetch_yahoo(symbol,start_date,end_date)
                if df is not None and 'Close'in df.columns:data[name]=df['Close'].reindex(target_index,method='ffill',limit=ff_limit)
            except:pass
        return data
    def _fetch_cboe_volatility_indices(self,start_date:str,end_date:str,target_index:pd.DatetimeIndex)->Dict[str,pd.Series]:
        import logging
        ...
        return data
    def _build_dollar_index_features(self,yahoo_data:Dict[str,pd.Series],target_index:pd.DatetimeIndex)->pd.DataFrame:
        features=pd.DataFrame(index=target_index)
        if 'Dollar_Index'not in yahoo_data:return features
        dxy=yahoo_data['Dollar_Index'];features['dxy']=dxy
        for window in[1,5,10,21,63]:features[f'dxy_ret_{window}d']=dxy.pct_change(window)*100
        dxy_returns=dxy.pct_change()
        for window in[10,21,63]:features[f'dxy_vol_{window}d']=dxy_returns.rolling(window).std()*np.sqrt(252)*100
        for window in[5,10,21]:
            features[f'dxy_velocity_{window}d']=dxy.diff(window)
            mom=dxy.diff(window);features[f'dxy_momentum_z_{window}d']=self._zscore(mom,63)
        ...
        bb_window=20;bb_ma=dxy.rolling(bb_window).mean();bb_std=dxy.rolling(bb_window).std()
        bb_upper=bb_ma+2*bb_std;bb_lower=bb_ma-2*bb_std
        features['dxy_bb_position_20d']=((dxy-bb_lower)/(bb_upper-bb_lower).replace(0,np.nan)).clip(0,1)
        ...
        return features
    def _build_commodity_vol_features(self,cboe_vol_data:Dict[str,pd.Series],vix:Optional[pd.Series],target_index:pd.DatetimeIndex)->pd.DataFrame:
        features=pd.DataFrame(index=target_index)
        if 'OVX'in cboe_vol_data:
            ovx=cboe_vol_data['OVX'];features['ovx']=ovx
            for window in[5,10,21]:
            ...
            for window in[10,21]:features[f'ovx_vol_{window}d']=ovx_returns.rolling(window).std()*np.sqrt(252)*100
            for window in[21,63,252]:
            ...
            features['ovx_regime']=self._regime_classify(ovx,bins=[0,20,30,45,150],labels=[0,1,2,3])
        if 'GVZ'in cboe_vol_data:
            gvz=cboe_vol_data['GVZ'];features['gvz']=gvz
            for window in[5,10,21]:
        ...
        return features
    def _build_commodity_features(self,yahoo_data:Dict[str,pd.Series],target_index:pd.DatetimeIndex)->pd.DataFrame:
        features=pd.DataFrame(index=target_index)
        if 'Gold_Futures'in yahoo_data:
            gold=yahoo_data['Gold_Futures'];features['gold']=gold
            for window in[5,10,21,63]:features[f'gold_ret_{window}d']=gold.pct_change(window)*100
            gold_returns=gold.pct_change()
            for window in[10,21,63]:features[f'gold_vol_{window}d']=gold_returns.rolling(window).std()*np.sqrt(252)*100
            for window in[21,63,252]:features[f'gold_zscore_{window}d']=self._zscore(gold,window)
        ...
        if 'Natural_Gas'in yahoo_data:
            ng=yahoo_data['Natural_Gas'];features['natgas']=ng
            for window in[5,10,21]:features[f'natgas_ret_{window}d']=ng.pct_change(window)*100
            ng_returns=ng.pct_change()
            features['natgas_vol_21d']=ng_returns.rolling(21).std()*np.sqrt(252)*100
            for window in[21,63]:features[f'natgas_zscore_{window}d']=self._zscore(ng,window)
        if 'Copper'in yahoo_data:
            copper=yahoo_data['Copper'];features['copper']=copper
            for window in[5,10,21,63]:features[f'copper_ret_{window}d']=copper.pct_change(window)*100
            for window in[21,63,252]:features[f'copper_zscore_{window}d']=self._zscore(copper,window)
            features['copper_momentum_63d']=copper.diff(63)
            features['copper_economic_signal']=(features['copper_momentum_63d']>0).astype(int)
        return features
    def _build_cross_asset_features(self,yahoo_data:Dict[str,pd.Series],cboe_vol_data:Dict[str,pd.Series],vix:Optional[pd.Series],target_index:pd.DatetimeIndex)->pd.DataFrame:
        features=pd.DataFrame(index=target_index)
        if 'Dollar_Index'in yahoo_data and 'Gold_Futures'in yahoo_data:
            dxy=yahoo_data['Dollar_Index'];gold=yahoo_data['Gold_Futures']
            for window in[21,63,126]:features[f'dxy_gold_corr_{window}d']=dxy.pct_change().rolling(window).corr(gold.pct_change())
        ...
        if 'GVZ'in cboe_vol_data and 'Gold_Futures'in yahoo_data:
            gvz=cboe_vol_data['GVZ'];gold=yahoo_data['Gold_Futures']
        ...
        if 'Copper'in yahoo_data and 'Dollar_Index'in yahoo_data:
            copper=yahoo_data['Copper'];dxy=yahoo_data['Dollar_Index']
            copper_weak=copper.pct_change(21)<0;dxy_strong=dxy.pct_change(21)>0
        ...
        if 'OVX'in cboe_vol_data:vol_components.append(self._zscore(cboe_vol_data['OVX'],63))
        if 'GVZ'in cboe_vol_data:vol_components.append(self._zscore(cboe_vol_data['GVZ'],63))
        ...
        return features
    def _build_regime_features(self,features:pd.DataFrame,yahoo_data:Dict[str,pd.Series],cboe_vol_data:Dict[str,pd.Series])->pd.DataFrame:
        ...
        if 'dxy_percentile_252d'in features.columns:
            ...
            regime_features['dxy_regime_numeric']=pd.cut(pct,bins=[0,25,75,100],labels=[0,1,2]).astype(float)
        if 'ovx'in features.columns:
            ovx=features['ovx']
            regime_features['commodity_vol_regime']=self._regime_classify(ovx,bins=[0,20,30,45,150],labels=[0,1,2,3])
        if 'dxy_gold_both_rally'in features.columns:regime_features['flight_to_safety_active']=features['dxy_gold_both_rally']
        risk_on_signals=[]
        if 'dxy_ret_21d'in features.columns:risk_on_signals.append(features['dxy_ret_21d']<0)
        if 'copper_ret_21d'in features.columns:risk_on_signals.append(features['copper_ret_21d']>0)
        if 'gold_ret_21d'in features.columns:risk_on_signals.append(features['gold_ret_21d']<0)
    ...
    def _regime_classify(self,series:pd.Series,bins:list,labels:list)->pd.Series:
        return pd.cut(series,bins=bins,labels=labels).astype(float)
def build_commodity_vol_features(data_fetcher,start_date:str,end_date:str,target_index:pd.DatetimeIndex,vix_series:Optional[pd.Series]=None)->pd.DataFrame:
    engineer=CommodityVolatilityEngineer(data_fetcher)
    return engineer.build_all_commodity_vol_features(start_date,end_date,target_index,vix_series)
