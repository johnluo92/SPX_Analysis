...
from config import TARGET_CONFIG,XGBOOST_CONFIG,TRAINING_END_DATE,REGIME_BOUNDARIES,REGIME_NAMES,QUALITY_FILTER_CONFIG,DIRECTION_CALIBRATION_CONFIG,ENSEMBLE_CONFIG,MAGNITUDE_PARAMS,DIRECTION_PARAMS,TRAIN_END_DATE,VAL_END_DATE
...
class SimplifiedVIXForecaster:
    ...
    def train(self,df,magnitude_features=None,direction_features=None,save_dir="models"):
        ...
        total_samples=len(X_mag);train_end_idx=df[df.index<=pd.Timestamp(TRAIN_END_DATE)].index[-1];train_end_idx=df.index.get_loc(train_end_idx);val_end_idx=df[df.index<=pd.Timestamp(VAL_END_DATE)].index[-1];val_end_idx=df.index.get_loc(val_end_idx)
        X_mag_train=X_mag.iloc[:train_end_idx+1];X_mag_val=X_mag.iloc[train_end_idx+1:val_end_idx+1];X_mag_test=X_mag.iloc[val_end_idx+1:];X_dir_train=X_dir.iloc[:train_end_idx+1];X_dir_val=X_dir.iloc[train_end_idx+1:val_end_idx+1];X_dir_test=X_dir.iloc[val_end_idx+1:]
        y_direction_train=df.iloc[:train_end_idx+1]["target_direction"];y_direction_val=df.iloc[train_end_idx+1:val_end_idx+1]["target_direction"];y_direction_test=df.iloc[val_end_idx+1:]["target_direction"];y_magnitude_train=df.iloc[:train_end_idx+1]["target_log_vix_change"];y_magnitude_val=df.iloc[train_end_idx+1:val_end_idx+1]["target_log_vix_change"];y_magnitude_test=df.iloc[val_end_idx+1:]["target_log_vix_change"]
        ...
        actual_train_end=df.iloc[:train_end_idx+1].index[-1];actual_val_end=df.iloc[:val_end_idx+1].index[-1];logger.info(f"Data: {df.index[0].date()} → {df.index[-1].date()} | Train through: {actual_train_end.date()} | Val through: {actual_val_end.date()}");logger.info(f"Split: Train={len(X_mag_train)} ({len(X_mag_train)/total_samples:.1%}) | Val={len(X_mag_val)} ({len(X_mag_val)/total_samples:.1%}) | Test={len(X_mag_test)} ({len(X_mag_test)/total_samples:.1%})");logger.info(f"Magnitude features: {len(self.magnitude_feature_names)} | Direction features: {len(self.direction_feature_names)}")
