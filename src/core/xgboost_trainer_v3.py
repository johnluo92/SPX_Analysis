...
class SimplifiedVIXForecaster:
    ...
    def _generate_diagnostics(self,X_test,y_direction_test,y_magnitude_test,save_dir,vix_test):
        save_path=Path(save_dir);save_path.mkdir(parents=True,exist_ok=True)
        try:
            fig,axes=plt.subplots(2,2,figsize=(14,10));fig.suptitle("Magnitude Model Performance",fontsize=16,fontweight="bold")
            ...
            ax.set_xlabel("Error (%)");ax.set_ylabel("Frequency");ax.set_title("Error Distribution");ax.legend();ax.grid(True,alpha=0.3)
            ax=axes[1,0];direction_from_mag=(y_mag_pred>0).astype(int);conf_bins=pd.cut(np.abs(test_pct_pred)/100,bins=[0,0.05,0.10,1.0],labels=["Low<5%","Med 5-10%","High>10%"])
            accs=[]
            ...
            ax.set_ylabel("Accuracy");ax.set_title("Direction by Confidence");ax.grid(True,alpha=0.3,axis="y")
