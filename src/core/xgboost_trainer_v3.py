...
class SimplifiedVIXForecaster:
    ...
    def train(self,df,magnitude_features=None,direction_features=None,save_dir="models"):
            ...
            train_mask=df.index<=pd.Timestamp(SPLIT_DATES['train_end']);val_mask=(df.index>pd.Timestamp(SPLIT_DATES['train_end']))&(df.index<=pd.Timestamp(SPLIT_DATES['val_end']));test_mask=df.index>pd.Timestamp(SPLIT_DATES['val_end'])
            X_mag_train=X_mag[train_mask];X_mag_val=X_mag[val_mask];X_mag_test=X_mag[test_mask];X_dir_train=X_dir[train_mask];X_dir_val=X_dir[val_mask];X_dir_test=X_dir[test_mask]
            y_direction_train=df[train_mask]["target_direction"];y_direction_val=df[val_mask]["target_direction"];y_direction_test=df[test_mask]["target_direction"];y_magnitude_train=df[train_mask]["target_log_vix_change"];y_magnitude_val=df[val_mask]["target_log_vix_change"];y_magnitude_test=df[test_mask]["target_log_vix_change"]
            ...
            total_samples=len(df);logger.info(f"Data: {df.index[0].date()} → {df.index[-1].date()}");logger.info(f"Split dates: Train ≤ {SPLIT_DATES['train_end']} | Val {SPLIT_DATES['train_end']} to {SPLIT_DATES['val_end']} | Test > {SPLIT_DATES['val_end']}");logger.info(f"Split: Train={len(X_mag_train)} ({len(X_mag_train)/total_samples:.1%}) | Val={len(X_mag_val)} ({len(X_mag_val)/total_samples:.1%}) | Test={len(X_mag_test)} ({len(X_mag_test)/total_samples:.1%})");logger.info(f"Magnitude features: {len(self.magnitude_feature_names)} | Direction features: {len(self.direction_feature_names)}")
