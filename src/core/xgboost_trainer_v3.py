...
class SimplifiedVIXForecaster:
    ...
    def predict(self,X,current_vix):
        ...
        magnitude_pct=(np.exp(magnitude_log)-1)*100;magnitude_pct=np.clip(magnitude_pct,-50,100);dir_proba=self.direction_model.predict_proba(X_dir)[0];direction="UP"if dir_proba[1]>0.5 else"DOWN";direction_confidence=float(max(dir_proba[0],dir_proba[1]));direction_probability=float(dir_proba[1]);expected_vix=current_vix*(1+magnitude_pct/100);current_regime=self._get_regime(current_vix);expected_regime=self._get_regime(expected_vix);regime_change=current_regime!=expected_regime;actionable=direction_confidence>XGBOOST_CONFIG["actionable_confidence_threshold"]
    ...
    def _generate_diagnostics(self,X_mag_test,X_dir_test,y_direction_test,y_magnitude_test,save_dir,vix_test):
        ...
        try:
            ...
            ax=axes[0,2];dir_proba=self.direction_model.predict_proba(X_dir_test)[:,1];conf_bins=pd.cut(np.maximum(dir_proba,1-dir_proba),bins=[0.5,0.6,0.7,0.8,1.0],labels=["50-60%","60-70%","70-80%","80-100%"]);accs=[]
    ...
    def _print_summary(self):
        print("\nTRAINING SUMMARY");print(f"Models: Magnitude Regressor + Direction Classifier | Training through: {TRAINING_END_DATE}");print(f"Magnitude: Train={self.metrics['magnitude']['train']['mae_pct']:.2f}% | Val={self.metrics['magnitude']['val']['mae_pct']:.2f}% | Test={self.metrics['magnitude']['test']['mae_pct']:.2f}% | Bias={self.metrics['magnitude']['test']['bias_pct']:+.2f}%")
