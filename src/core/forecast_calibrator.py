...
class ForecastCalibrator:
    ...
    def fit_from_database(self,database):
        ...
        logger.info(f"Exponential weights: min={df['weight'].min():.3f}, max={df['weight'].max():.3f}, mean={df['weight'].mean():.3f}")
        ...
        self.fitted=True;logger.info("✅ Calibration complete");return True
    def _fit_regime_cohort_corrections(self,df):
        ...
        for regime in regimes:
            for cohort in cohorts:
                ...
                bias=np.average(subset["error"],weights=subset["weight"]);self.corrections["regime_cohort"][key]=float(bias);count+=1
        logger.info(f"Regime+Cohort: {count} corrections fitted")
    def _fit_regime_corrections(self,df):
        ...
        for regime in regimes:
            ...
            if len(subset)<self.min_samples:continue
            bias=np.average(subset["error"],weights=subset["weight"]);self.corrections["regime"][regime]=float(bias);count+=1
        logger.info(f"Regime-only: {count} corrections fitted")
    def _fit_cohort_corrections(self,df):
        ...
        for cohort in cohorts:
            ...
            if len(subset)<self.min_samples:continue
            bias=np.average(subset["error"],weights=subset["weight"]);self.corrections["cohort"][cohort]=float(bias);count+=1
        logger.info(f"Cohort-only: {count} corrections fitted")
    def _fit_global_correction(self,df):
        bias=np.average(df["error"],weights=df["weight"]);self.corrections["global"]=float(bias)
        logger.info(f"Global: bias={bias:+.3f}%")
    ...
    def save(self,output_dir="models"):
        ...
        if not self.fitted:logger.warning("⚠️  Saving unfitted calibrator")
