...
def validate_feature_quality(features:pd.DataFrame,feature_list:list,detector_name:str)->Tuple[list,dict]:
    quality_report={}
    valid_features=[]
    ...
    if removed_count>0:print(f"  {detector_name}: Removed {removed_count}/{len(feature_list)} low-quality features")
    return valid_features,quality_report
def calculate_robust_anomaly_score(raw_score:float,training_distribution:np.ndarray,min_percentile:float=0.05,max_percentile:float=0.95)->float:
...
def calculate_coverage_penalty(coverage:float,min_coverage:float=0.7)->float:
    if coverage>=min_coverage:return 1.0
    else:
...
class MultiDimensionalAnomalyDetector:
    def __init__(self,contamination:float=0.01,random_state:int=RANDOM_STATE):
        ...
        self.importance_config={"method":"shap" if SHAP_AVAILABLE else "permutation","n_samples":500,"n_repeats":1}
    def _calculate_feature_importance(self,detector,X_scaled,baseline_scores,feature_names,verbose=False):
        if SHAP_AVAILABLE and self.importance_config["method"]=="shap":
            try:return self._calculate_shap_importance(detector,X_scaled,feature_names,verbose)
            except Exception as e:
                if verbose:warnings.warn(f"SHAP failed, using permutation: {str(e)[:100]}")
    ...
    def _calculate_permutation_importance(self,detector,X_scaled,baseline_scores,feature_names,verbose=False):
        try:
            ...
            for i,feature_name in enumerate(feature_names):
                ...
                except:importances[feature_name]=0.0
            total=sum(importances.values())
            if total>0:importances={k:v/total for k,v in importances.items()}
            else:importances={name:1.0/len(feature_names) for name in feature_names}
            return {k:v if np.isfinite(v) else 1.0/len(feature_names) for k,v in importances.items()}
        except:return {name:1.0/len(feature_names) for name in feature_names}
    def calculate_statistical_thresholds(self)->dict:
        ...
        thresholds={"moderate":float(np.percentile(scores,85)),"high":float(np.percentile(scores,92)),"critical":float(np.percentile(scores,98))}
        self.logger.info(f"Statistical Thresholds: Moderate={thresholds['moderate']:.4f}, High={thresholds['high']:.4f}, Critical={thresholds['critical']:.4f}")
        return thresholds
    def train(self,features:pd.DataFrame,verbose:bool=True):
        ...
        for name,feature_list in self.feature_groups.items():
            try:
                if verbose:self.logger.debug(f"Training: {name}")
                available_features=[f for f in feature_list if f in features.columns]
                valid_features,quality_report=validate_feature_quality(features,available_features,name)
                self.feature_quality_reports[name]=quality_report
                if len(valid_features)<3:
                    if verbose:self.logger.warning(f"{name}: Only {len(valid_features)} valid features, skipping")
                    continue
                X=features[valid_features].fillna(0)
                scaler=RobustScaler()
                X_scaled=scaler.fit_transform(X)
                detector=IsolationForest(contamination=self.contamination,random_state=self.random_state,n_estimators=100,max_samples="auto",n_jobs=-1)
                ...
                self.feature_importances[name]=self._calculate_feature_importance(detector,X_scaled,training_scores,valid_features,verbose=False)
            except Exception as e:
                if verbose:self.logger.error(f"{name}: Training failed - {str(e)[:100]}")
                continue
        if verbose:self.logger.debug("Generating 5 random subspace detectors...")
        all_features=list(available_cols)
        for i in range(5):
            try:
                subspace_size=np.random.randint(8,15)
                subspace_features=np.random.RandomState(self.random_state+i).choice(all_features,size=min(subspace_size,len(all_features)),replace=False).tolist()
                name=f"random_{i+1}"
                valid_features,quality_report=validate_feature_quality(features,subspace_features,name)
                self.feature_quality_reports[name]=quality_report
                if len(valid_features)<3:continue
                X=features[valid_features].fillna(0)
                scaler=RobustScaler()
                X_scaled=scaler.fit_transform(X)
                detector=IsolationForest(contamination=self.contamination,random_state=self.random_state+i,n_estimators=100,max_samples="auto",n_jobs=-1)
                detector.fit(X_scaled)
                training_scores=detector.score_samples(X_scaled)
                self.detectors[name]=detector
                self.scalers[name]=scaler
                ...
                if verbose:self.logger.warning(f"Random subspace {i+1} failed: {str(e)[:50]}")
        self.trained=True
    ...
    def detect(self,features:pd.DataFrame,verbose:bool=True)->dict:
        if not self.trained:raise ValueError("Detector not trained")
        results={"domain_anomalies":{},"random_anomalies":{},"ensemble":{},"data_quality":{}}
        scores=[]
        detector_weights=[]
        active_detectors=0
        missing_features=[]
        for name,detector in self.detectors.items():
            feature_list=self.random_subspaces[int(name.split("_")[1])-1] if name.startswith("random_") else [f for f in self.feature_groups[name] if f in features.columns]
            ...
            try:
                ...
                result_dict={"score":float(adjusted_score),"raw_anomaly_score":float(anomaly_score),"percentile":float(adjusted_score*100),"level":level,"coverage":float(coverage),"coverage_penalty":float(coverage_penalty),"weight":float(coverage*coverage_penalty)}
                if name.startswith("random_"):results["random_anomalies"][name]=result_dict
                else:results["domain_anomalies"][name]=result_dict
            except Exception as e:missing_features.append({"detector":name,"coverage":coverage,"error":str(e)})
        if scores:
            ...
            results["ensemble"]={"score":ensemble_score,"std":float(np.std(scores)),"max_anomaly":float(np.max(scores)),"min_anomaly":float(np.min(scores)),"n_detectors":active_detectors,"mean_weight":float(np.mean(weights)),"weighted":True}
        results["data_quality"]={"active_detectors":active_detectors,"total_detectors":len(self.detectors),"missing_features":missing_features,"weight_stats":{"mean":float(np.mean(weights)) if len(weights)>0 else 0.0,"min":float(np.min(weights)) if len(weights)>0 else 0.0,"max":float(np.max(weights)) if len(weights)>0 else 0.0},"feature_quality_summary":self._get_quality_summary()}
        return results
    def _get_quality_summary(self)->dict:
        ...
        for detector_name,quality_report in self.feature_quality_reports.items():
            ...
            summary[detector_name]={"valid_ratio":valid_count/total_count if total_count>0 else 0.0,"valid_count":valid_count,"total_count":total_count}
        return summary
    def calculate_historical_persistence_stats(self,ensemble_scores:np.ndarray,dates:Optional[pd.DatetimeIndex]=None,threshold:float=None)->Dict:
        if threshold is None:threshold=self.statistical_thresholds.get("high",0.78) if isinstance(self.statistical_thresholds,dict) else 0.78
        if isinstance(ensemble_scores,list):ensemble_scores=np.array(ensemble_scores)
        ...
        if dates is not None and current_streak>0 and PYTZ_AVAILABLE:
            try:
                ...
                if hasattr(last_date,"tz") and last_date.tz is not None:last_date_normalized=last_date.tz_convert("US/Eastern").normalize()
                if last_date_normalized==today_et:
                    market_closed=now_et.hour>=16
                    if not market_closed and is_anomalous[-1]:current_streak=max(0,current_streak-1)
            except Exception as e:warnings.warn(f"Streak timing correction failed: {e}")
        total_anomaly_days=int(is_anomalous.sum())
        return {"current_streak":int(current_streak),"mean_duration":float(np.mean(streaks)) if streaks else 0.0,"max_duration":int(np.max(streaks)) if streaks else 0,"median_duration":float(np.median(streaks)) if streaks else 0.0,"total_anomaly_days":total_anomaly_days,"anomaly_rate":float(total_anomaly_days/len(ensemble_scores)) if len(ensemble_scores)>0 else 0.0,"num_episodes":len(streaks),"threshold_used":float(threshold)}
    def get_top_anomalies(self,result:dict,top_n:int=3)->list:
        all_anomalies=[]
        for name,data in result.get("domain_anomalies",{}).items():all_anomalies.append((name,data["score"]))
        for name,data in result.get("random_anomalies",{}).items():all_anomalies.append((name,data["score"]))
    ...
    def get_feature_contributions(self,detector_name:str,top_n:int=5)->List[Tuple[str,float]]:
        if detector_name not in self.feature_importances:return []
    ...
    def classify_anomaly(self,score:float,method:str="statistical")->tuple:
        thresholds=self.statistical_thresholds if self.statistical_thresholds else {"moderate":0.70,"high":0.78,"critical":0.88}
        if "moderate_ci" in thresholds:thresholds={"moderate":thresholds["moderate"],"high":thresholds["high"],"critical":thresholds["critical"]}
        if len(self.training_ensemble_scores)>0:
