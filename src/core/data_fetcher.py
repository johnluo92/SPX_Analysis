...
class UnifiedDataFetcher:
    ...
    MONTHLY_SERIES = {
        'UNRATE', 'CPIAUCSL', 'CPILFESL', 'PCEPI', 'PCEPILFE',
        'UMCSENT', 'INDPRO', 'PAYEMS', 'M1SL', 'M2SL', 'WALCL', 'WTREGEN'
    }
    DAILY_SERIES = {
        'DGS1MO', 'DGS3MO', 'DGS6MO', 'DGS1', 'DGS2', 'DGS5', 'DGS10', 'DGS30',
        'DFF', 'SOFR', 'BAMLH0A0HYM2', 'BAMLC0A0CM', 'VIXCLS',
    ...
    FRED_SERIES_GROUPS = {
        'TREASURIES': {
            ...
            'DGS1': '1Y_Treasury', 'DGS2': '2Y_Treasury', 'DGS5': '5Y_Treasury',
            'DGS10': '10Y_Treasury', 'DGS30': '30Y_Treasury',
        },
        'TREASURY_SPREADS': {
        ...
        'FED_RATES': {
            'DFF': 'Fed_Funds_Effective', 'SOFR': 'SOFR',
        },
        'CREDIT_SPREADS': {
    ...
    def _should_update_fred_series(self, series_id: str, last_date: str) -> bool:
        ...
        if series_id in self.QUARTERLY_SERIES:
            ...
            last_quarter_month = max([m for m in quarter_end_months.keys() if m <= last_dt.month])
            ...
            if now.month > last_quarter_month:
                ...
                quarter_end = datetime(last_dt.year if last_quarter_month < 12 else last_dt.year + 1,
                                      last_quarter_month + 1 if last_quarter_month < 12 else 1, 1)
        ...
        if series_id in self.MONTHLY_SERIES:
            ...
            if now.month == last_dt.month + 1 or (now.month == 1 and last_dt.month == 12):
    ...
    def fetch_fred(self, series_id: str, start_date: str = None, end_date: str = None,
        ...
        if incremental and cache_path.exists():
            ...
            if last_date:
                ...
                self.logger.info(f"FRED:{series_id}: Checking for updates from {start_date}")
    ...
    def fetch_fred_series(self, series_id: str, start_date: str = None, 
                          end_date: str = None, incremental: bool = True) -> Optional[pd.Series]:
    ...
    def fetch_all_fred_series(self, start_date: str = None, end_date: str = None, 
                              incremental: bool = True) -> Dict[str, pd.Series]:
        ...
        for group_name, series_dict in self.FRED_SERIES_GROUPS.items():
            for series_id, readable_name in series_dict.items():
                ...
                series = self.fetch_fred(series_id, start_date, end_date, incremental=incremental)
    ...
    def fetch_yahoo(self, symbol: str, start_date: str = None, end_date: str = None,
        ...
        if incremental and cache_path.exists():
            ...
            if last_date:
                ...
                if days_since_cache < 3 and (not end_date or pd.to_datetime(end_date) >= last_dt):
                    try:
                        cached_df = pd.read_parquet(cache_path)
                        if not cached_df.empty:
                            return cached_df
        ...
        try:
            ...
            df = ticker.history(start=start_date or '2000-01-01', end=end_date, auto_adjust=True)
