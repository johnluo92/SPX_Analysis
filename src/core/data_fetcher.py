...
class DataFetchLogger:
    def __init__(self,name:str="DataFetcher"):
        self.logger=logging.getLogger(name);self.logger.handlers.clear();self.logger.propagate=False;h=logging.StreamHandler();h.setFormatter(logging.Formatter("%(asctime)s [%(levelname)s] %(message)s",datefmt="%H:%M:%S"));self.logger.addHandler(h);self.logger.setLevel(logging.INFO)
...
class UnifiedDataFetcher:
    ...
    def __init__(self,cache_dir:str="./data_cache",cboe_data_dir:str="./CBOE_Data_Archive"):
        ...
        if not self.fred_api_key:self.logger.warning("FRED API key missing")
    ...
    def fetch_fred(self,series_id:str,start_date:str=None,end_date:str=None,incremental:bool=True)->Optional[pd.Series]:
        ...
        if incremental and cache_path.exists():
            ...
            if last_date:
                ...
                start_date=(pd.to_datetime(last_date)+timedelta(days=1)).strftime("%Y-%m-%d");self.logger.info(f"FRED:{series_id}: Checking updates from {start_date}")
        try:
            params={"series_id":series_id,"api_key":self.fred_api_key,"file_type":"json","observation_start":start_date or"1990-01-01","observation_end":end_date or datetime.now().strftime("%Y-%m-%d")};response=requests.get(self.fred_base_url,params=params,timeout=10);response.raise_for_status();data=response.json()
            ...
            df.to_parquet(cache_path);self.logger.info(f"FRED:{series_id}: Updated ({len(df)} rows)");return df[series_id]if series_id in df.columns else None
            ...
            return None
    def fetch_fred_series(self,series_id:str,start_date:str=None,end_date:str=None,incremental:bool=True)->Optional[pd.Series]:
        return self.fetch_fred(series_id,start_date,end_date,incremental)
    def fetch_all_fred_series(self,start_date:str=None,end_date:str=None,incremental:bool=True)->Dict[str,pd.Series]:
        all_series={};fetched_count=0
        for group_name,series_dict in self.FRED_SERIES_GROUPS.items():
            for series_id,readable_name in series_dict.items():
                cache_path=self.cache_dir/f"fred_{series_id}.parquet";was_cached=cache_path.exists();series=self.fetch_fred(series_id,start_date,end_date,incremental=incremental)
                if series is not None:
                    all_series[readable_name]=series
                    if was_cached and cache_path.exists():cache_age=(datetime.now()-datetime.fromtimestamp(cache_path.stat().st_mtime)).days;fetched_count+=1 if cache_age<1 else 0
        if all_series:msg=f"FRED: {len(all_series)} series";msg+=f" ({fetched_count} updated)"if fetched_count>0 else"";self.logger.info(msg)
        return all_series
    def fetch_yahoo(self,symbol:str,start_date:str=None,end_date:str=None,incremental:bool=True)->Optional[pd.DataFrame]:
        ...
        if is_historical_request and cache_path.exists():
            try:
                ...
                if not cached_df.empty:
                    start_dt=pd.to_datetime(start_date)if start_date else cached_df.index[0];cache_start=cached_df.index[0];cache_end=cached_df.index[-1];cache_covers_range=(cache_start<=start_dt)and(cache_end>=end_dt)
                    if cache_covers_range:filtered_df=cached_df[(cached_df.index>=start_dt)&(cached_df.index<=end_dt)];self.logger.info(f"Yahoo:{symbol}: Historical data ({start_dt.date()} to {end_dt.date()})");return filtered_df if not filtered_df.empty else None
                    else:self.logger.info(f"Yahoo:{symbol}: Cache incomplete (has {cache_start.date()} to {cache_end.date()}, need {start_dt.date()} to {end_dt.date()})")
            except Exception as e:self.logger.warning(f"Yahoo:{symbol}: Cache read failed - {e}")
        fetch_start=start_date or"2000-01-01"
        if is_historical_request:incremental=False;self.logger.info(f"Yahoo:{symbol}: Fetching full historical range ({fetch_start} to {end_date})")
        if incremental and cache_path.exists():
            last_date=self._get_last_date_from_cache(cache_path)
            if last_date:
                ...
                if business_days_diff==0 and hours_since_update<1:
                    try:
                        cached_df=pd.read_parquet(cache_path)
                        if not cached_df.empty:self.logger.info(f"Yahoo:{symbol}: Using cache (updated {int(hours_since_update*60)}m ago)");return cached_df
                    except:pass
                elif business_days_diff==1 and now.hour<16:
                    try:
                        cached_df=pd.read_parquet(cache_path)
                        if not cached_df.empty:self.logger.info(f"Yahoo:{symbol}: Using cache (1 bday behind, pre-market)");return cached_df
                    except:pass
                fetch_start=(last_dt-timedelta(days=2)).strftime("%Y-%m-%d");self.logger.info(f"Yahoo:{symbol}: Fetching from {fetch_start} ({business_days_diff} bdays behind)")
        try:
            ticker=yf.Ticker(symbol);fetch_end=end_date if end_date else(datetime.now()+timedelta(days=2)).strftime("%Y-%m-%d");df=ticker.history(start=fetch_start,end=fetch_end,auto_adjust=True)
            if df.empty:
                if cache_path.exists():
                    try:
                        cached_df=pd.read_parquet(cache_path)
                        if not cached_df.empty:self.logger.info(f"Yahoo:{symbol}: No new data, using cache");return cached_df
                    except:pass
                return None
            ...
            if df is None:
                ...
                return None
            if incremental and cache_path.exists():df=self._merge_with_cache(df,cache_path)
            df.to_parquet(cache_path);last_date_str=df.index[-1].strftime("%Y-%m-%d");self.logger.info(f"Yahoo:{symbol}: {len(df)} rows (latest: {last_date_str})")
            if is_historical_request:start_dt=pd.to_datetime(start_date)if start_date else df.index[0];df=df[(df.index>=start_dt)&(df.index<=end_dt)]
            return df
        except Exception as e:
            if cache_path.exists():
                try:
    ...
    def fetch_cboe_series(self,symbol:str)->Optional[pd.Series]:
        if symbol not in self.CBOE_FILES:return None
        file_path=self.cboe_data_dir/self.CBOE_FILES[symbol]
        if not file_path.exists():self.logger.warning(f"CBOE:{symbol}: File not found");return None
        try:
            ...
            self.logger.info(f"CBOE:{symbol}: {len(df)} rows");series=df.iloc[:,0]if len(df.columns)>0 else None
            if series is not None:series=series.astype(np.float64)
            return series
        except Exception as e:self.logger.error(f"CBOE:{symbol}: {str(e)[:100]}");return None
    def fetch_all_cboe(self)->Dict[str,pd.Series]:
        ...
        if series_dict:self.logger.info(f"CBOE: {len(series_dict)} series loaded")
        return series_dict
    def fetch_fomc_calendar(self,start_year:int,end_year:int)->Optional[pd.DataFrame]:
    ...
    def update_fomc_calendar_from_csv(self,csv_path:str)->Optional[pd.DataFrame]:
        try:
            ...
            existing_df=existing_df.reset_index();existing_df.rename(columns={existing_df.columns[0]:"date"},inplace=True);combined=pd.concat([existing_df,import_df]);combined=combined.drop_duplicates(subset=["date"],keep="last");combined=combined.sort_values("date");cache_path=self.cache_dir/"fomc_calendar.csv";combined.to_csv(cache_path,index=False);added_count=len(combined)-len(existing_df);self.logger.info(f"FOMC: Calendar updated: {len(combined)} total meetings")
            if added_count>0:self.logger.info(f"FOMC: Added {added_count} new meetings")
