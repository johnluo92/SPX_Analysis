...
class SeriesFetcher:
    def __init__(self, series_id, api_key_filename='fred_api_key.txt'):
        self.series_id = series_id
        self.base_url = 'https://api.stlouisfed.org/fred/series/observations'
        
        # Determine the base path based on operating system
        if platform.system() == 'Windows':
            base_path = r"C:\Users\jl078\OneDrive\Desktop\WorkSpace\SPX_Analysis\src"
        else:  # macOS
            base_path = os.path.expanduser("~/Desktop/GitHub/SPX_Analysis/src")
            
        # Construct the full path to the API key file
        self.api_key_file = os.path.join(base_path, api_key_filename)
        
        # Ensure the directory exists
        if not os.path.exists(os.path.dirname(self.api_key_file)):
            raise FileNotFoundError(f"Directory not found: {os.path.dirname(self.api_key_file)}")
