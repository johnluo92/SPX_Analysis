"""
Sector Rotation Model v3.5 - Pragmatic Hybrid
Complete implementation ready to run

PHILOSOPHY:
- Keep v3.4's better macro factors (CL=F, DX-Y.NYB are better than USO/UUP)
- Restore 126d window but ONLY for sectors that need it
- Slightly reduce regularization from v3.4 (but not back to original)
- Accept that ~55% accuracy is realistic for this approach
- Focus on probability calibration, not chasing accuracy

CHANGES FROM v3.4:
1. ✅ Add 126d window for XLF, XLE, XLRE, XLB, XLP (long-horizon sectors)
2. ✅ Sector-specific hyperparameters (not one-size-fits-all)
3. ✅ Slightly lighter regularization (max_depth 4→5 for some sectors)
4. ✅ Keep v3.4's macro factors (they ARE better despite complexity)
"""
...
class SectorRotationFeatures:
    """Feature engineering with sector-aware window selection."""
    ...
    def calculate_relative_strength(self,
                                    ...
                                    long_horizon_sectors: List[str] = None) -> pd.DataFrame:
        """
        Calculate RS with sector-specific windows.
        
        Long-horizon sectors get [21, 63, 126]
        Short-horizon sectors get [21, 63]
        """
        if long_horizon_sectors is None:
            # Sectors that benefit from longer windows
            long_horizon_sectors = ['XLF', 'XLE', 'XLRE', 'XLB', 'XLP', 'XLU']
        
        print("🔧 Calculating relative strength...")
        print(f"   💡 Long-horizon sectors: {', '.join(long_horizon_sectors)}")
        ...
        for ticker in sectors.columns:
            ...
            # Determine windows for this sector
            if ticker in long_horizon_sectors:
                windows = [21, 63, 126]
            else:
                windows = [21, 63]
    ...
    def calculate_macro_changes(self,
        ...
        """
        Macro changes - velocity only, no acceleration.
        Uses v3.4's macro factors (better than USO/UUP).
        """
        print("🔧 Calculating macro changes...")
        
        features = pd.DataFrame(index=macro.index)
        
        # Velocity for all factors except 5Y Treasury
        for col in macro.columns:
            if col == '5Y Treasury':
                continue  # Only use in slope calculation
            
            for window in windows:
                change = macro[col].pct_change(window) * 100
                features[f'{col}_change_{window}d'] = change
        
        # Yield curve slope (critical for financials)
        if '10Y Treasury' in macro.columns and '5Y Treasury' in macro.columns:
            ...
            print(f"   ✅ Created Yield Curve Slope features")
    ...
    def add_vix_features(self, vix: pd.Series) -> pd.DataFrame:
        ...
        features['VIX_Regime'] = pd.cut(
            vix, bins=[0, 15, 25, 35, 100], labels=[0, 1, 2, 3]
        ).astype(int)
    ...
    def combine_features(self,
        ...
        print("\n" + "="*70)
        print("FEATURE ENGINEERING v3.5 - PRAGMATIC HYBRID")
        print("="*70)
        print("Changes: Restore 126d for long-horizon sectors, keep v3.4 macros")
        ...
        all_features = pd.concat([
            rs_features, macro_features, season_features, vix_features
        ], axis=1)
...
class SectorRotationModel:
    """Model with sector-specific hyperparameters."""
    
    def __init__(self, random_state: int = 42):
        """
        Initialize with pragmatic sector-aware configs.
        
        PHILOSOPHY: Not too tight, not too loose.
        """
        self.sector_categories = {
            'FINANCIALS': ['XLF'],
            'MACRO_SENSITIVE': ['XLE', 'XLP', 'XLRE', 'XLB'],
        ...
        self.sector_configs = {
            'XLF': {  # Financials - need flexibility for yield curve
                'n_estimators': 150,
                'max_depth': 5,           # v3.4 was 4 (too restrictive)
                'min_samples_split': 40,
                'min_samples_leaf': 35,
                'max_features': 'sqrt',
            },
            'MACRO_SENSITIVE': {  # Lightest regularization
                'n_estimators': 150,
                'max_depth': 5,
                'min_samples_split': 40,
                'min_samples_leaf': 30,
            ...
            'SENTIMENT_DRIVEN': {  # Heaviest regularization (they need it)
                'n_estimators': 150,
                'max_depth': 4,
    ...
    def get_sector_category(self, ticker: str) -> str:
        """Get category for a ticker."""
        for category, tickers in self.sector_categories.items():
            if ticker in tickers:
                return category
        return 'MIXED'
    
    def get_config_for_sector(self, ticker: str) -> Dict:
        """Get hyperparameters for specific sector."""
        if ticker == 'XLF':
            return self.sector_configs['XLF']
        
        category = self.get_sector_category(ticker)
        return self.sector_configs[category]
    ...
    def analyze_feature_importance(self,
                                   ...
                                   min_importance: float = 0.008) -> List[str]:
        """
        Feature selection with 0.8% threshold (less aggressive than v3.4's 1%).
        """
        print("\n" + "="*70)
    ...
    def train_models(self,
        ...
        """Train with sector-specific configs."""
        print("\n" + "="*70)
        print("TRAINING MODELS - v3.5 SECTOR-SPECIFIC")
        print("="*70)
        
        if use_feature_selection and self.selected_features is None:
            self.selected_features = self.analyze_feature_importance(
                features, targets, min_importance=0.008
            )
        ...
        for target_col in y_all.columns:
            ...
            category = self.get_sector_category(ticker)
            config = self.get_config_for_sector(ticker)
            
            print(f"\n🌲 {ticker} ({category})")
            print(f"   Config: depth={config['max_depth']}, "
                  f"split={config['min_samples_split']}, "
...
def test_v35_model():
    """Test v3.5 - pragmatic hybrid approach."""
    print("\n" + "="*70)
    print("SECTOR ROTATION MODEL v3.5 - PRAGMATIC HYBRID")
    print("="*70)
    print("\nKey Changes:")
    print("  1. Restore 126d window for long-horizon sectors")
    print("  2. Keep v3.4's macro factors (they ARE better)")
    print("  3. Sector-specific hyperparameters")
    print("  4. Slightly lighter regularization")
    print("  5. Less aggressive feature selection (0.8% vs 1.0%)")
    ...
    # Compare to v3.4
    print("\n" + "="*70)
    print("COMPARISON TO v3.4")
    print("="*70)
    print("\nDid XLF improve? (It should have more signal now with 126d window)")
    print("Did macro-sensitive sectors maintain performance?")
    print("Did sentiment sectors stay stable (they should)?")
